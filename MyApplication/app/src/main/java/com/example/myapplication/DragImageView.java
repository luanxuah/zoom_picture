...
public class DragImageView extends ImageView {
	...
	private int initial_W, initial_H;// 当前图片宽高
	 ...
	 * @param initial_H 高度
	 */
	public void setInitialSize(int initial_W, int initial_H){
		/** 获取图片宽高 **/
	...
	public void setScale(float scale) {
		...
		Log.i("doctorlog","       scale="+scale+"     disX="+disY+"       disY="+disY);
		// 放大
		if (scale > 1 && this.getWidth() <= MAX_W) {
			 ...
			 */
			if (current_Top <= 0 && current_Bottom >= initial_H) {
						Log.e("jj", "屏幕高度=" + this.getHeight());
				isControl_V = true;// 开启垂直监控
			} else {
				isControl_V = false;
			}
			if (current_Left <= 0 && current_Right >= initial_W) {
		...
		// 缩小
			...
			if (isControl_V && current_Top > 0) {
				...
				if (current_Bottom < initial_H) {
					current_Bottom = initial_H;
					isControl_V = false;// 关闭垂直监听
				}
			}
			// 下边越界
			if (isControl_V && current_Bottom < initial_H) {
				current_Bottom = initial_H;
			...
			if (isControl_H && current_Left >= 0) {
				...
				if (current_Right <= initial_W) {
					current_Right = initial_W;
					isControl_H = false;// 关闭
				}
			}
			// 右边越界
			if (isControl_H && current_Right <= initial_W) {
				current_Right = initial_W;
	...
	public void setScale(boolean zoomIn){
		if (zoomIn){
			if (this.getWidth()<2*initial_W){
				current_Left = (int) (- 0.5*initial_W);
				current_Right = (int) (1.5*initial_W);
				current_Top = (int) (- 0.5*initial_H);
				current_Bottom = (int) (1.5*initial_H);
				isControl_V = true;// 开启垂直监控
				isControl_H = true;// 开启水平监控
			}else{
				current_Left = (int) (-1.5*initial_W);
				current_Right = (int) (2.5*initial_W);
				current_Top = (int) (- 1.5*initial_H);
				current_Bottom = (int) (2.5*initial_H);
				isControl_V = true;// 开启垂直监控
				isControl_H = true;// 开启水平监控
			}
			...
			if (this.getWidth()>2*initial_W){
				...
				current_Top = 0;
				current_Bottom = initial_H;
				isControl_V = false;// 开启垂直监控
				isControl_H = false;// 开启水平监控
			}
		}
		this.setFrame(current_Left, current_Top, current_Right,
				current_Bottom);
	...
	public void doScaleAnim() {
		myAsyncTask = new MyAsyncTask(initial_W, this.getWidth(),
				this.getHeight());
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		private int initial_W, current_Width, current_Height;
		...
		public MyAsyncTask(int initial_W, int current_Width, int current_Height) {
			super();
			this.initial_W = initial_W;
